...
sound_crash = pygame.mixer.Sound("mus/Crash.wav")
pygame.mixer.music.load("mus/Wicked_Things.wav")
...
def crash(img):
    ...
    while True:
        ...
        button("AGAIN", 150, 450, 100, 50, green, bright_green, lambda: game_loop(img))
...
def choose_car(img, position):
    ...
    if display_width * (position - 1) / 3 < mouse_hover[0] < display_width * position / 3:
        pygame.draw.rect(gameDisplay, bright_yellow,
                         (display_width * (position - 1) / 3, 0, display_width / 3, display_height))
...
def paused():
    ...
    while pausing:
        ...
        button("CONTINUE", (display_width / 2 - 150), 450, 300, 50, green, bright_green, unpaused)
